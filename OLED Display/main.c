...
static char display_buf[32];  // scratch buffer for sprintf
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // Line 3: CH1 voltage (internal sensor raw voltage)
	  sprintf(display_buf, "V_ch1: %.3f V", voltage_ch1);
	  ssd1306_SetCursor(0, 32);
	  ssd1306_WriteString(display_buf, Font_7x10, White);

	  // Line 4: CH2 voltage (PA1 external)
...
/* USER CODE BEGIN 4 */
/*
 * STM32-only voltage test stage:
 * - Reads PA0 as ADC1_IN0
 * - Reads PA1 as ADC1_IN1
 * - Stores raw ADC counts in raw_ch1/raw_ch2
 * - Stores converted voltages in voltage_ch1/voltage_ch2
 */
/* USER CODE END 4 */
