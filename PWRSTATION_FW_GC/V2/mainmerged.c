...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Power-station monitor — INA228 (V/I/P/E/Q) +
  *                   2-channel NTC thermistor (T1/T2)
  *
  *  INA228 (I2C1 — PB6/PB7)
  *    Measures bus voltage, shunt current, power, energy, charge, die temp.
  *
  *  NTC thermistor channels (ADC1) — unchanged from mainthrmls.c
  *    PA0 -> ADC_CH0 -> T1   (NTC to GND, 10k pull-up to 3.3V)
  *    PA1 -> ADC_CH1 -> T2   (NTC to GND, 10k pull-up to 3.3V)
  *
  *  NOTE: OLED_RST_Pin (PA0) is defined in main.h but the SSD1306 has an
  *        internal power-on reset — oled.c never drives it. PA0/PA1 are
  *        therefore free for analog ADC use.
  *
  ...
  *    Row 2   I:   3.4560 A
  *    Row 3   P:  42.345 W
  *    Row 4   T:   27.5 C      <- INA228 die temperature
  *    Row 5   E: 1234.5  J     <- slow task
  *    Row 6   Q:  567.8  C     <- slow task
  *    Row 7   T1:xx.x T2:xx.xC <- NTC thermistors
  ******************************************************************************
  */
...
/* -- NTC thermistor parameters (from mainthrmls.c) ------------------------- */
...
volatile float  temp_t1    = 0.0f;   /* PA0 temperature */
volatile float  temp_t2    = 0.0f;   /* PA1 temperature */
...
/* ── NTC helpers (unchanged from mainthrmls.c) ────────────────────────────── */
...
static void app_init(void)
{
    ...
    if (ina_s != INA228_OK)
    {
        ...
        if (oled_s == OLED_OK)
        {
            ...
            oled_print_str(0, 2, "NTC-only mode");
            oled_flush();
            HAL_Delay(2000);
        }
        return;
...
static void app_fast_task(void)
{
    ...
    {
        oled_printf_row(1, "V:    --- V");
        oled_printf_row(2, "I:    --- A");
        oled_printf_row(3, "P:    --- W");
        oled_printf_row(4, "T:    --- C");
    }

    /* NTC T1 (PA0) and T2 (PA1) */
    ...
    /* Row 7 — "T1:25.3 T2:31.7C" */
    oled_printf_row(7, "T1:%.1f T2:%.1fC",
                    (double)temp_t1, (double)temp_t2);

    oled_flush();
}

/**
  * @brief  Slow task — 5000 ms cadence.  Energy/charge -> rows 5-6.
  */
static void app_slow_task(void)
{
    if (!g_ina_ok) return;
    ...
    oled_printf_row(6, "Q:%7.3fC",  g_charge_C);
    oled_flush();
}

/**
  * @brief  I2C1 bus recovery — call before MX_I2C1_Init().
  */
static void I2C1_BusRecovery(void)
{
...
static void MX_ADC1_Init(void)
{
    ...
    /* Default to CH0 (PA0/T1) — ADC_SelectChannel() overrides at runtime */
...
static void MX_GPIO_Init(void)
{
    ...
    /*
     * PA0 (OLED_RST_Pin) and PA1 are left as analog inputs.
     * oled.c does NOT drive the RST pin — the SSD1306 has an internal
     * power-on reset, so PA0/PA1 are free for ADC_CHANNEL_0/1 (T1/T2).
     */
